...
public class SecurityUserController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<SecurityUserDTO> getById(@PathVariable Integer id) {
        return ResponseEntity.ok(service.findById(id));
    }

    @PostMapping
    public ResponseEntity<SecurityUserDTO> create(@RequestBody SecurityUserDTO dto) {
        SecurityUserDTO created = service.create(dto);
