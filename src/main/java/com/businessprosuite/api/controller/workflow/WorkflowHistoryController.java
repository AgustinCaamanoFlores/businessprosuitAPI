...
@RestController
@RequestMapping("/api/workflow-histories")
...
public class WorkflowHistoryController {
    ...
    @GetMapping
    public ResponseEntity<List<WorkflowHistoryDTO>> getAll() {
        return ResponseEntity.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<WorkflowHistoryDTO> getById(@PathVariable Integer id) {
        return ResponseEntity.ok(service.findById(id));
    }

    @PostMapping
    public ResponseEntity<WorkflowHistoryDTO> create(@RequestBody WorkflowHistoryDTO dto) {
        WorkflowHistoryDTO created = service.create(dto);
        return ResponseEntity.status(201).body(created);
    }

    @PutMapping("/{id}")
    public ResponseEntity<WorkflowHistoryDTO> update(
            @PathVariable Integer id,
            @RequestBody WorkflowHistoryDTO dto) {
